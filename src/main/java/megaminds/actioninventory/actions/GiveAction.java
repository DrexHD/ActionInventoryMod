...
public final class GiveAction extends BasicAction {
	...
	public void accept(@NotNull ActionInventoryGui gui) {
		...
		var lootContext = new LootWorldContext.Builder(p.getEntityWorld())
				.add(LootContextParameters.THIS_ENTITY, p)
				.add(LootContextParameters.ORIGIN, p.getEntityPos())
				.luck(p.getLuck())
				.build(LootContextTypes.ADVANCEMENT_REWARD);

		Arrays.stream(lootTables)
		.map(id-> Optional.ofNullable(p.getEntityWorld().getServer().getRegistryManager().getOrThrow(RegistryKeys.LOOT_TABLE).get(id)).orElse(LootTable.EMPTY).generateLoot(lootContext))
